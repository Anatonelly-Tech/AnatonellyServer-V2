...
const responsibleFreightController = {
    create: async (req, res) => {
        try {
            const { name, email, phone, contactWay, department, password, isAdmin } = req.body;
            ...
            res.status(200).json({ response, msg: 'Responsável do Frete criado com sucesso!' });
        } catch (error) {
            if (error.code == '11000') {
                res.status(400).json({ error: 'Id do responsável já existe!' });
            }
            console.log(error);
        }   
    },
    ...
    delete: async (req, res) => {
        try {
            const idResponsible = req.params.idResponsible;
            const response = await ResponsibleFreightModel.findOneAndDelete( {idResponsible} );
            if(!response){
                return res.status(400).json({ error: "Id do responsável não existe!" });
            }
            res.status(200).json({ msg: 'Responsável do Frete deletado com sucesso!'});
        } catch (err) {
            console.log(err);
        }
    },
